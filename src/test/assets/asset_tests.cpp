...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of STPCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("RVN", type));
        BOOST_CHECK(!IsAssetNameValid("STP", type));
        BOOST_CHECK(!IsAssetNameValid("STPCOIN", type));

        //- Versions of STPCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("STP.COIN", type));
        BOOST_CHECK(IsAssetNameValid("STP_COIN", type));
        BOOST_CHECK(IsAssetNameValid("RVNSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERRVN", type));
        BOOST_CHECK(IsAssetNameValid("STPSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDESTP", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_STPS", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("STP", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
