...
rm -rf ~/stp.network ~/sign ~/release
...
find ${DISTNAME}/bin -type f -executable -exec ../stp.network/contrib/devtools/split-debug.sh {} {} {}.dbg \;
find ${DISTNAME}/lib -type f -exec ../stp.network/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
mkdir -p unsigned
mv ~/stp.network/stp-*setup-unsigned.exe unsigned/
...
mkdir -p ~/stp.network/depends/SDKs
cp ~/MacOSX10.11.sdk.tar.gz ~/stp.network/depends/SDKs/MacOSX10.11.sdk.tar.gz
...
cd ~/stp.network
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "STP.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
