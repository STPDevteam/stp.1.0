...
git clone https://github.com/STPDevteam/stp.network.git
mkdir -p release
cd stp.network/depends
...
sudo find ${DISTNAME}/bin -type f -executable -exec ../stp.network/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../stp.network/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo stp-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find stp-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/stp.network
...
DISTNAME=stp-2.0.1
sudo apt install -y curl g++-aarch64-linux-gnu g++-7-aarch64-linux-gnu gcc-7-aarch64-linux-gnu binutils-aarch64-linux-gnu g++-arm-linux-gnueabihf g++-7-arm-linux-gnueabihf gcc-7-arm-linux-gnueabihf binutils-arm-linux-gnueabihf g++-7-multilib gcc-7-multilib binutils-gold git pkg-config autoconf libtool automake bsdmainutils ca-certificates python
git clone https://github.com/stpproject/coin
mkdir -p release
mkdir -p ~/wrapped
...
cp -f stp-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf stp.network/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/stp.network/-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/stp-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer stp-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer stp-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf stp.network/contrib/windeploy ~/sign/win32
...
#transfer stp-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/stp.network/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/STPDevteam/stp.network
mkdir ~/stp.network/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder stp.network/depends/SDKs
cd stp.network/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/stp.network
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "STP-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/stp-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer stp-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
